...
$username = "user"; // Promijenite ovo u vaše korisničko ime baze podataka
$password = "password"; // Promijenite ovo u vašu lozinku baze podataka
$database = "klinikabosna"; // Promijenite ovo u ime vaše baze podataka
...
if (isset($_SESSION['ime']) && isset($_SESSION['prezime'])) {
    ...
    $sql = "SELECT Uloga FROM osobe WHERE ime = '$ime' AND Prezime = '$prezime'";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        $tip_korisnika = $row["Uloga"];
